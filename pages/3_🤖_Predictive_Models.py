...
if 'datasets' in st.session_state and st.session_state.datasets:
    ...
    if model_type == "STEM Interest Prediction":
        ...
        selected_features = st.multiselect(
            ...
            available_features,
            default=['stem_interest_pre', 'engagement_score', 'quest_completion_rate', 
                    'skill_progression', 'collaboration_events']
        )
        
        if st.button("Train Models", type="primary") and len(selected_features) > 0:
            # Prepare data
            X = full_data[selected_features]
            y = full_data['stem_interest_post']
            ...
            # Learning curves
            with st.expander("View Learning Curves"):
                fig_learning = go.Figure()
                
                train_sizes = np.linspace(0.1, 1.0, 10)
                
                for name, model in {'Ridge': Ridge(), 'Random Forest': RandomForestRegressor(n_estimators=50)}.items():
